<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<!DOCTYPE html>
<!--[if lt IE 7 ]><html class="no-js ie ie67 ie678 ie6789" lang="en"><![endif]-->
<!--[if IE 7 ]><html class="no-js ie ie67 ie678 ie6789 ie7" lang="en"><![endif]-->
<!--[if IE 8 ]><html class="no-js ie ie678 ie6789 ie8" lang="en"><![endif]-->
<!--[if IE 9 ]><html class="no-js ie ie6789 ie9" lang="en"><![endif]-->
<!--[if !(IE)]><!-->
<html lang="en">
  <!--<![endif]-->
<head>
<title>会员注册</title>
<jsp:include page="../common/WerResources.jsp"></jsp:include>
<script src="/site/js/usercenter/RebateUtil.js"></script>
<script type="text/javascript">
	var rebate = initRebateByString("${rebate.rebateStr}");
	$(function() {
		fanDianJiSuan();
	});
</script>
<script src="/site/js/usercenter/createSubUser.js"></script>
</head>
<body class="layout-two-column unibet custom-background">
	<div id="container">
		<jsp:include page="../common/WerHeader.jsp"></jsp:include>
		<div id="content">
			<div id="top">
				<div class="inner"></div>
			</div>

			<div id="main" role="main">

				<div class="main-container">
					<!-- 左侧菜单开始 -->
					<jsp:include page="AgencyMenu.jsp"></jsp:include>
					<!--彩种类型-->
					<div id="column-primary" class="stack gutter-col size4of5">
						<div class="header-image">
							<h2 class="heading">会员注册</h2>
						</div>

						<article class="rich-text-element main-column">
							<article class="widget registration-form">
								<form action="" method="post" id="createUserForm"
									style="height:auto;">
									<input type="hidden" name="flag" value="insert" /> <input
										type="hidden" name="controller" value="user" /> <input
										type="hidden" name="action" value="adduser" />
									<div
										style="margin-left:12px;margin-right:12px;margin-top:12px;">
										<table class="formTable">
											<tbody>
												<tr>
													<td style="text-align: right;">会员类型：</td>
													<td colspan="2">
														<%----%>
														<span><input id="daili" type="radio" name="type" value="代理"/>代理</span>
														&nbsp;&nbsp;&nbsp;&nbsp;
														<span><input id="huiyuan" type="radio" checked name="type" value="会员"/>会员</span>
													</td>
												</tr>
												<tr>
													<td style="text-align: right;">用户名：</td>
													<td colspan="2">
														<input type="text" id="userName" class="field invalid" style="width:260px"
														name="userName" onblur="checkUser(this.value)" value=""
														onclick="$('#userchk').html('')" /> <span class="red">(由字母或数字组成的6-16个字符,不能连续四位相同的字符,首字不能以0或者o开头)</span>
														<p id="userchk"></p>
													</td>
												</tr>
												<tr>
													<td style="text-align: right;">密码：</td>
													<td colspan="2"><input type="password" name="userpass" class="field invalid" style="width:260px"
														id="userpass" value="" onclick="$('#pwdchk').html('')"/>
														<span class="red">(由字母和数字组成6-16个字符；且必须包含数字和字母)</span>
														<p id="pwdchk"></p>
													</td>
												</tr>
												<tr>
													<td  style="text-align: right;">用户昵称：</td>
													<td colspan="2"><input type="text" name="nickname" class="field invalid" style="width:260px"
														id="nickname" onclick="$('#nicknamechk').html('')" /> <span
														class="red">(由2至6个字符组成)</span>
														<p id="nicknamechk"></p>
													</td>
												</tr>
												<tr >
													<td  style="text-align: right;">返点：</td> 
													<td>
													<input type="text" class="fandianinput field invalid" size="4" style="width:260px"
														value="" name="keeppoint" id="keeppoint">
														<span	class="red">(代理返点<span style="display:none" id='keeppoint_min'>0.5</span> 
														<span style="display:none" id='keeppoint_max'>5.0</span> )
												</tr>
												<tr class="lastTr">
													<td></td>
													<td><input type="button" class="btn" value="提交注册"
														onclick="saveUser()">
													</td>
												</tr>
											</tbody>
										</table>
									</div>
								</form>
							</article>
							<div id="top">
								<div class="inner"></div>
							</div>
							<article class="widget bingo-game-chooser">
								<div class="box" style="border-radius: 0px;">
									<h1 class="box-heading"
										style="font-size: 16px;color:#333;margin-bottom: 0px;">彩种奖金</h1>
									<!--彩种类型-->
									<div id="nav-sub">
										<nav>
											<ul id="gameType" class="color-theme-1">
												<c:forEach items="${gameType}" var ="item" >
													<li value="${item}" id="two1" class="active"><a
															style="padding-left: 13px;padding-right: 13px;">${item.desc }</a></li>
												</c:forEach>
											</ul>
										</nav>
									</div>
								</div>
								<!--彩种类型 结束-->
								<div id="allbackset" class="overflowHidden"
									style="display:block;">
									<table
										class="bingo-game-chooser-table table-decoration-7 color-theme-4">
										<tr>
											<th>玩法</th>
											<th>最高</th>
											<th>最低</th>
											<th>返点</th>
											<th>浮动</th>
										</tr>
									</table>
								</div>
							</article>
						</article>
					</div>
				</div>
			</div>
		</div>
		<jsp:include page="../common/WerFooter.jsp"></jsp:include>
	</div>
</body>
</html>